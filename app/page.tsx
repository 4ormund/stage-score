  ...
  if (status === 'loading') {
        return (
                <div className="min-h-screen flex flex-col items-center justify-center">
                        <div className="w-8 h-8 border-2 border-gray-600 border-t-purple-400 rounded-full animate-spin"></div>div>
                        <p className="mt-4 text-gray-500">Loading...</p>p>
                </div>div>
              )
  }
          ...
          return (
                  <div className="min-h-screen flex flex-col items-center justify-center p-6">
                          <div className="text-center mb-8">
                                    <StageLogo />
                                    <h1 className="text-2xl font-bold text-white mt-4 mb-2">Stage Score</h1>h1>
                                    <p className="text-gray-500">Connect your X account to view your $SEED score</p>p>
                          </div>div>
                  
                          <button
                                      onClick={() => signIn('twitter')}
                                      className="flex items-center gap-3 px-6 py-3 rounded-lg transition-all purple-btn text-white font-medium"
                                    >
                          ...
                          <div className="mt-8 text-center text-gray-600 text-sm max-w-md">
                                    <p>Your Stage Score is calculated based on:</p>p>
                                    <ul className="mt-2 space-y-1 text-left pl-4">
                                                <li>• Purchase history of $SEED</li>li>
                                                <li>• Hold time duration</li>li>
                                                <li>• Net trading value (P/L)</li>li>
                                    </ul>ul>
                          </div>div>
                  </div>div>
                )
                ...
                return (
                      <div className="min-h-screen p-6">
                            <div className="max-w-2xl mx-auto">
                              {/* Header */}
                                    <div className="flex items-center justify-between mb-6 pb-4 border-b divider">
                                              <div className="flex items-center gap-4">
                                                          ...
                                                          <div>
                                                                        <h1 className="text-lg font-semibold text-white">{user.name}</h1>h1>
                                                                        <p className="text-gray-500 text-sm">@{user.username}</p>p>
                                                          </div>div>
                                              </div>div>
                                              <div className="flex items-center gap-3">
                                                          <span className="px-3 py-1 rounded text-xs text-gray-300 border border-gray-700">Rank #26,327</span>span>
                                                          <span className="px-3 py-1 rounded text-xs text-gray-300 border border-gray-700">Top 9.2%</span>span>
                                                          <button className="px-4 py-2 rounded-lg text-white font-medium text-sm purple-btn">Score card</button>button>
                                                          <button
                                                                          onClick={() => signOut()}
                              ...
                              {/* Score Section */}
                                    <div className="border-b divider">
                                              <div className="flex items-center justify-between py-4">
                                                          <span className="text-gray-300">Overall</span>span>
                                                          <div className="flex items-center gap-4">
                                                                        <div className="w-32 h-1.5 rounded-full overflow-hidden bg-gray-800">
                                                                                        <div
                                                                                                            className="h-full rounded-full bg-stage-purple"
                                                                                                            style={{ width: `${(score.overall / 1000) * 100}%` }}
                                                                                                          ></div>div>
                                                                        </div>div>
                                                                        <span className="text-white text-xl font-light tabular-nums w-16 text-right">{score.overall}</span>span>
                                                          </div>div>
                                              </div>div>
                                    </div>div>
                            
                                    <div className="flex items-center justify-between py-4 border-b divider">
                                              <span className="text-gray-400">Purchase</span>span>
                                              <span className="text-white text-lg tabular-nums">{score.breakdown.purchase}</span>span>
                                    </div>div>
                            
                                    <div className="flex items-center justify-between py-4 border-b divider">
                                              <span className="text-gray-400">Hold Time</span>span>
                                              <span className="text-white text-lg tabular-nums">{score.breakdown.holdTime}</span>span>
                                    </div>div>
                            
                                    <div className="flex items-center justify-between py-4 border-b divider">
                                              <span className="text-gray-400">Net Trading</span>span>
                                              <span className="text-white text-lg tabular-nums">{score.breakdown.netTrading}</span>span>
                                    </div>div>
                              ...
                              {/* Holdings Table */}
                                    <div className="mt-6">
                                              <div className="grid grid-cols-6 gap-4 py-3 text-xs text-gray-500 uppercase tracking-wider border-b divider">
                                                          <div>TOKEN</div>div>
                                                          <div className="text-right">BALANCE</div>div>
                                                          <div className="text-right">VALUE</div>div>
                                                          <div className="text-right">AVG BUY</div>div>
                                                          <div className="text-right">CURRENT</div>div>
                                                          <div className="text-right">P/L %</div>div>
                                              </div>div>
                                              <div className="grid grid-cols-6 gap-4 py-4 items-center border-b divider">
                                                          <div className="flex items-center gap-3">
                                                                        ...
                                                                        <div>
                                                                                        <div className="flex items-center gap-2">
                                                                                                          <span className="text-white font-medium">{TOKEN_SYMBOL}</span>span>
                                                                                                          <span className="text-blue-400 text-xs">✓</span>span>
                                                                                          </div>div>
                                                          ...
                                                          <div className="text-right text-white">{(mockOnChainData.currentBalance / 1000).toFixed(1)}K</div>div>
                                                          ...
                                                          <div className="text-right text-white">${priceData.value.toFixed(6)}</div>div>
                              ...
                              {/* Transaction History */}
                                    <div className="mt-6">
                                              <div className="py-3 text-sm text-gray-500 border-b divider">Transaction History</div>div>
                                    ...
                                    <div key={i} className="flex items-center justify-between py-3 border-b divider">
                                                  <div className="flex items-center gap-3">
                                                                  ...
                                                                  <div>
                                                                                    <span className="text-white text-sm">Buy</span>span>
                                                  ...
                                                  <div className="text-right">
                                                                  ...
                                                                  <div className="text-gray-500 text-xs">@ ${tx.priceAtBuy.toFixed(6)}</div>div>
                                                  </div>div>
                                    </div>div>
                                  ))}
                                    </div>div>
                              ...
                              {/* Footer */}
                                    <div className="mt-8 pt-4 border-t divider flex items-center justify-between">
                                              <StageLockup />
                                              <span className="text-gray-500 text-sm">@{user.username}</span>span>
                                    </div>div>
                            </div>div>
                      </div>div>
                    )
